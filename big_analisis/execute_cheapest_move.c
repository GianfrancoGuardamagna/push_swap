...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/09 14:40:09 by gguardam          #+#    #+#             */
/*   Updated: 2025/09/09 14:40:11 by gguardam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_stacks(t_stack **stack_a, t_stack **stack_b, int cheapest)
{
	...
	target = find_target(*stack_a, cheapest);
	if ((cost_in_stack(*stack_b, cheapest) > 0) && \
(cost_in_stack(*stack_a, target) > 0))
	{
		while ((*stack_a)->value != target && (*stack_b)->value != cheapest)
			rr(stack_a, stack_b);
	}
	else if ((cost_in_stack(*stack_b, cheapest) < 0) && \
(cost_in_stack(*stack_a, target) < 0))
	{
		while ((*stack_a)->value != target && (*stack_b)->value != cheapest)
			rrr(stack_a, stack_b);
